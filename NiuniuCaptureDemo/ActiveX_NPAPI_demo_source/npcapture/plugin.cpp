...
static NPIdentifier sCapture_id;
...
{
  return name == sFoo_id || name == sCapture_id;
}
...
string CPlugin::GetDllPath()
{
	char szPath[MAX_PATH] = {0};
	GetModuleFileName(GetModuleHandle("npcapture.dll"),szPath, MAX_PATH);  
  ...
  m_pScriptableObject(NULL)
{
  ...
  string strPath = GetDllPath() + "\\NiuniuCapture.dll";
//  ::MessageBox(NULL, strPath.c_str(), "", 0);
  m_hModule = LoadLibrary(strPath.c_str());
  ...
  sCapture_id = NPN_GetStringIdentifier("screencapture");
