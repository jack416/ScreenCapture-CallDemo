========================================================================
    活动模板库：ActiveXWrapper 项目概述
========================================================================

应用程序向导已为您创建了此 ActiveXWrapper 项目，您可以在此基础之上
编写自己的动态链接库 (DLL)。
...
ActiveXWrapper.idl
    ...
    和 coclass 的 IDL 定义。
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明                 (ActiveXWrapper.h)
        GUID 定义                                (ActiveXWrapper_i.c)
        一个类型库                               (ActiveXWrapper.tlb)
        封送代码                                 (ActiveXWrapper_p.c 和 dlldata.c)

ActiveXWrapper.h
    此文件包含在 ActiveXWrapper.idl 中定义的项的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 ActiveXWrapper.pch 的预编译头 (PCH) 文件
...
ActiveXWrapperps.vcproj
	...
	dlldata.c、ActiveXWrapper_i.c 和 ActiveXWrapper_p.c，它们。
